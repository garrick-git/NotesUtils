...
/**
 * NotesViewPool class provides a storage of Notes Views. This storage allows not
...
public class NotesViewPool {
	...
	public static NotesViewPool getInstance() {
		if (instance == null) {
			synchronized (NotesViewPool.class) {
				NotesViewPool inst = instance;
				if (inst == null) {
					synchronized (NotesViewPool.class) {
						inst = new NotesViewPool();
					}
					instance = inst;
				}
			}
